...
``-DCMAKE_INSTALL_PREFIX=/opt/FAangband-2.0`` would place all the files within
/opt/FAangband-2.0 or its subdirectories).  For finer-grained placement of the
...
To build FAangband to be run in-place, run this::
...
Once built, go to build/game/ subdirectory and start faangband by::
...
TODO: except for recent versions (after FAangband 2.0.1) you likely need to
...
That leaves ``build/FAangband.ipa``.  The app is not code signed, so putting it
